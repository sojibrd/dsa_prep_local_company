...
`dsa_prep` (লোকাল ফোল্ডার + GitHub repo + লাইভ সাইট) মুছে তার জায়গায় তিনটা স্বতন্ত্র প্রজেক্ট — `dsa_prep_local_company`, `dsa_prep_remote_company`, `dsa_prep_global_company` — প্রতিটা নিজের ফোল্ডার, নিজের public repo, নিজের GitHub Pages সাইট। প্রতিটা `switch_in_6_month_local_company`-এর **পুরো গড়ন** মানে: `docs/`-এ দিনভিত্তিক plan, "আজ"-এর পাতা, rail + মোবাইলে drawer, `/day/`, `/block/`, `/review/` (🔁 ১/৩/৭/২১), `/rules/`, 🧠 নীতির chip, ⚑ মাইলফলক, Theme contract, dark-only। কনটেন্ট `brainstorming/dsa-prep-how-many-paths.md`-এর তিনটা পথ, আর প্রতিটা কাজ `learning_to_learn`-এর নীতি (principle, lies, pillars, science, techniques) থেকে। পাশাপাশি `switch_in_6_month_local_company` থেকে প্রবলেমের তালিকা সরে গিয়ে শুধু ৩০′-এর DSA ঘর থাকবে।
...
* **`learning_to_learn` মেনে চলা:** `switch_in_6_month_local_company`-এর মতো — কাজের শেষে `🧠 (নাম · নাম)`, 🔁 স্বয়ংক্রিয় ঝালাই, বসার নিয়মে interleaving/Feynman/deliberate practice; ব্যাখ্যা `app/lib/principles.ts`-এ, লিংক `learning_to_learn` সাইটে। কনটেন্ট কপি নয়।
...
* **১ · তারিখের উৎস:** ফাইলে শুধু দিনের নম্বর (`### দিন ০০৭ · শিরোনাম`), তারিখ নয়। শুরুর তারিখ ইউজার সাইটে একবার বসান (`localStorage`); "আজ" = শুরু + দিনের নম্বর। শুরুর তারিখ না থাকলে হোমে শুধু "কবে শুরু করবেন?"। লোকালের প্রস্তাবিত শুরু **২০২৬-০৯-২১** (= `switch_in_6_month_local_company`-এর দিন ০০৮)। *কারণ:* রিমোট/গ্লোবালের তারিখ কোথাও লেখা নেই, বানানো যাবে না; তিন প্রজেক্টে এক নিয়ম। `switch_in_6_month_local_company`-এর "দিন গোনা কোডে নয়" নিয়ম এখানে জেনেবুঝে ভাঙা।
* **২ · লোকালের ৩০টা প্রবলেমের মালিক:** শুধু `dsa_prep_local_company`। `switch_in_6_month_local_company`-এ প্রবলেমের নাম ও 🔁 সরে `- [ ] ৩০′ DSA — আজকের প্রবলেম dsa_prep_local_company-এ` হবে; `00-rules.md`-এর "DSA-র বসা" লোকাল প্রজেক্টের নিয়মে সরে এক লাইনের লিংক। *কারণ:* টিক ও ঝালাই এক জায়গায়। DSA শুরু ০৯-২১, তাই হারানোর মতো টিক নেই।
...
* **কাজের ক্রম:** নতুন তিনটা লাইভ → `switch_in_6_month_local_company` বদল → রেফারেন্স হালনাগাদ → **সবার শেষে** `dsa_prep` মোছা, মোছার ঠিক আগে আবার অনুমতি।
* **repo:** তিনটাই public।
...
* লোকালের ৩০টা প্রবলেম = `switch_in_6_month_local_company/docs/01-ignition.md` ও `02-stories.md`-এর বর্তমান তালিকা হুবহু, একই ক্রমে।
...
1. **ভিত তৈরি (লোকাল):** `switch_in_6_month_local_company`-এর `app/`, config, workflow কপি করে `dsa_prep_local_company` বানানো (create-next-app নয় — এই plan ফাইল ফোল্ডারে আছে, আর গড়ন হুবহু লাগবে)। `node_modules/next/dist/docs/` দেখে নেওয়া।
...
5. **লোকালের কনটেন্ট:** `docs/00-rules.md` (DSA-র বসা, ঝালাই, যা করবেন না — `switch_in_6_month_local_company` থেকে সরানো), ব্লক ফাইল, `patterns/`-এ টপিক ১–৫-এর ২৩টা প্যাটার্ন; `principles.ts`-এ দরকারি নাম।
6. **লোকাল build, README, `.github/workflows/deploy.yml`** → `gh repo create sojibrd/dsa_prep_local_company --public` → push → Pages চালু → লাইভ যাচাই।
...
9. **`switch_in_6_month_local_company` বদল:** `01-ignition.md`, `02-stories.md`, `03-applications.md`-এ DSA কাজ → ৩০′ ঘর + লিংক; `00-rules.md`-এর DSA অংশ → লিংক; `dsa_prep` উল্লেখ (README, কম্পোনেন্টের মন্তব্য) হালনাগাদ → build → push।
