...
- **Cross-device sync (pilot, ২০২৬-০৯-১৮, শুধু এই repo-তে — এখনো `_remote_company`/`_global_company`-তে কপি হয়নি)** — localStorage/IndexedDB সবসময় primary (offline-এ পুরোপুরি কাজ করে); sync key বসানো থাকলে `app/hooks/useSync.ts` background-এ একটা শেয়ার্ড Supabase project-এ push/pull করে (`app/lib/supabase.ts`, স্কিমা `supabase/schema.sql`)। Conflict resolution: পুরো blob-এর last-write-wins, `<prefix>:v1:meta`-এর `updatedAt` দিয়ে। Sync key auth-less — একটা random UUID, `sync:v1:key` (prefix ছাড়া, origin-শেয়ার্ড, তাই সব সাইটে একই key কাজ করে)। GitHub Actions build-এ `SUPABASE_URL`/`SUPABASE_ANON_KEY` repo secret লাগে (`.github/workflows/deploy.yml`)। Pilot validate হলে বাকি ৯টা প্রজেক্টে (dsa_prep বাকি দুটো + behavioural_interview_* + switch_in_6_month_* + leave_management) একই প্যাটার্ন কপি হবে।
